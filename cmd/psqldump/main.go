...
func printUsage(w io.Writer) {
	fmt.Fprintln(w, "psqldump dumps a remote PostgreSQL DB and creates a self-restoring Docker Compose setup.")
	fmt.Fprintln(w)
	fmt.Fprintln(w, "Usage:")
	fmt.Fprintln(w, "  psqldump <command> [flags]")
	fmt.Fprintln(w)
	fmt.Fprintln(w, "Commands:")
	fmt.Fprintln(w, "  dump      Dump the remote database to a local SQL file")
	fmt.Fprintln(w, "  build     Build a Docker image with the dump baked in")
	fmt.Fprintln(w, "  compose   Generate a docker-compose.yml for the restored database")
	fmt.Fprintln(w, "  all       Run dump, build, and compose in sequence")
	fmt.Fprintln(w)
	fmt.Fprintln(w, "Run 'psqldump <command> --help' for command flags.")
}

func printCommandUsage(w io.Writer, command string) {
	fmt.Fprintf(w, "Usage: psqldump %s [flags]\n\n", command)
	fmt.Fprintln(w, "Flags:")
	fmt.Fprintln(w, "  -H, --host string             PostgreSQL host (default \"localhost\")")
	fmt.Fprintln(w, "  -P, --port int                PostgreSQL port (default 5432)")
	fmt.Fprintln(w, "  -U, --user string             PostgreSQL user (default \"postgres\")")
	fmt.Fprintln(w, "  -W, --password string         PostgreSQL password (or set PGPASSWORD)")
	fmt.Fprintln(w, "  -d, --dbname string           Database name (required)")
	fmt.Fprintln(w, "  -o, --out string              Output directory (default \".\")")
	fmt.Fprintln(w, "  -E, --external-port int       Host port for the generated compose file (default 5432)")
	fmt.Fprintln(w, "      --pg-version string       PostgreSQL major version (e.g. 16). Empty = auto-detect from server")
}
